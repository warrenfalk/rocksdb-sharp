...
if [[ $OSINFO == *"MSYS"* || $OSINFO == *"MINGW"* ]]; then
	...
	(cd rocksdb && {
		...
		sed -i '/add_subdirectory(tools)/d' CMakeLists.txt || warn "Patching of CMakeLists.txt failed"

		mkdir -p build
		(cd build && {
			cmake -G "Visual Studio 15 2017 Win64" -WITH_TESTS=OFF -DOPTDBG=1 -DGFLAGS=0 -DSNAPPY=1 -DPORTABLE=1 -DWITH_AVX2=0 -DWITH_MD_LIBRARY=OFF .. || fail "Running cmake failed"
